...
const EditorPage = () => {
  ...
  const [hook, setHook] = useState("5 exercícios que vão transformar seu shape em 30 dias");
  const [caption, setCaption] = useState("Se você está cansado de treinar sem ver resultado, presta atenção nesses 5 exercícios que são verdadeiros divisores de água.");
  const [hashtags, setHashtags] = useState("#fabrik #treino #fitness #shape");
  const [cta, setCta] = useState("Salve esse post e comece amanhã 💪");

  return (
    <div>
      <h1 className="text-3xl font-serif mb-1">Editor</h1>
      <p className="text-muted-foreground mb-8">
        {jobId ? `Editando job #${jobId}` : "Edite o conteúdo antes de aprovar"}
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-5">
          ...
          <div className="flex gap-3 pt-4">
            <Button className="bg-brand hover:bg-brand-hover text-primary-foreground flex-1">
              Salvar e Aprovar
            </Button>
            <Button
              variant="ghost"
        ...
        <div className="bg-card border border-border rounded-xl p-4 h-fit">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full bg-brand flex items-center justify-center text-xs font-bold text-primary-foreground">
              AG
            </div>
